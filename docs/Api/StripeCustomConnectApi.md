...
# **refresh**
> object refresh($link_request_id, $app_id)
...
try {
    $result = $apiInstance->refresh($link_request_id, $app_id);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling StripeCustomConnectApi->refresh: ', $e->getMessage(), PHP_EOL;
}
?>
```
