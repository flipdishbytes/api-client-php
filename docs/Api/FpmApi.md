...
try {
    $apiInstance->deleteFpmForStore($store_id);
} catch (Exception $e) {
    echo 'Exception when calling FpmApi->deleteFpmForStore: ', $e->getMessage(), PHP_EOL;
}
?>
```
...
# **getFpmForStore**
> \Flipdish\\Client\Models\TelephonyConfig getFpmForStore($store_id)
...
try {
    $result = $apiInstance->getFpmForStore($store_id);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling FpmApi->getFpmForStore: ', $e->getMessage(), PHP_EOL;
}
?>
```
...
[**\Flipdish\\Client\Models\TelephonyConfig**](../Model/TelephonyConfig.md)
