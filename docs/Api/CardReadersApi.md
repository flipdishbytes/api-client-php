...
# **getReader**
> \Flipdish\\Client\Models\RestApiResultCardReader getReader($reader_id, $app_id)
...
try {
    $result = $apiInstance->getReader($reader_id, $app_id);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling CardReadersApi->getReader: ', $e->getMessage(), PHP_EOL;
}
?>
```
