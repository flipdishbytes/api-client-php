...
class SubscriptionsApi
{
    ...
    protected function getSubscriptionInvoicesRequest($app_id, $subscription_id, $limit = null, $starting_after_id = null)
    {
        ...
        if ($app_id === null || (is_array($app_id) && count($app_id) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $app_id when calling getSubscriptionInvoices'
            );
        }
        // verify the required parameter 'subscription_id' is set
        if ($subscription_id === null || (is_array($subscription_id) && count($subscription_id) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $subscription_id when calling getSubscriptionInvoices'
            );
        }

        $resourcePath = '/api/v1.0/{appId}/subscriptions/{subscriptionId}/invoices';
        $formParams = [];
        $queryParams = [];
        $headerParams = [];
        $httpBody = '';
